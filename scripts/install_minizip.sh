...
cmake -S minizip-ng-4.2.2 -B build-minizip -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX="$INSTALL_PREFIX" -DCMAKE_PREFIX_PATH="$INSTALL_PREFIX" -DZLIB_LIBRARY="$INSTALL_PREFIX"/lib/libzs.a -DBUILD_SHARED_LIBS=OFF -DMZ_FETCH_LIBS=OFF -DMZ_PPMD=OFF -DMZ_BUILD_TESTS=OFF
