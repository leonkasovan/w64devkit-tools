...
if pkg-config --exists libjpeg 2>/dev/null || [ -f "$INSTALL_PREFIX/lib/libjpeg.a" ]; then
...
cmake -S libjpeg-turbo-3.1.0 -B build-libjpeg -DCMAKE_INSTALL_PREFIX="$INSTALL_PREFIX" -DENABLE_SHARED=OFF -DENABLE_STATIC=ON -DWITH_JPEG8=ON -DWITH_SIMD=OFF -DWITH_TURBOJPEG=OFF -DWITH_JAVA=OFF
