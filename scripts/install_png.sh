...
cmake -S libpng-1.6.58 -B build_png -DCMAKE_BUILD_TYPE=Release -DPNG_SHARED=OFF -DPNG_TOOLS=OFF -DPNG_TESTS=OFF -DPNG_STATIC=ON -DCMAKE_INSTALL_PREFIX="$INSTALL_PREFIX" -DCMAKE_PREFIX_PATH="$INSTALL_PREFIX" -DZLIB_LIBRARY="$INSTALL_PREFIX"/lib/libzs.a
